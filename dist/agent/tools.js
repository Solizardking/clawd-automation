    ...
    return [
        // ── VM/Sandbox Tools ──
        {
            ...
            execute: async (args, ctx) => {
                ...
                if (forbidden)
                    return forbidden;
                const result = await ctx.clawd.exec(command, args.timeout || 30000);
        ...
        },
        {
            ...
            execute: async (args, ctx) => {
                ...
                await ctx.clawd.writeFile(filePath, args.content);
        ...
        },
        {
            ...
            execute: async (args, ctx) => {
                return await ctx.clawd.readFile(args.path);
            },
        },
        {
            name: "expose_port",
            ...
            execute: async (args, ctx) => {
                const info = await ctx.clawd.exposePort(args.port);
        ...
        },
        {
            ...
            execute: async (args, ctx) => {
                await ctx.clawd.removePort(args.port);
        ...
        // ── Clawd API Tools ──
        {
            ...
            execute: async (_args, ctx) => {
                const balance = await ctx.clawd.getCreditsBalance();
        ...
        },
        {
            ...
            parameters: { type: "object", properties: {} },
            execute: async (_args, ctx) => {
                const { getUsdcBalance } = await import("../shell/x402.js");
        ...
        },
        {
            ...
            execute: async (args, ctx) => {
                const info = await ctx.clawd.createSandbox({
                    name: args.name,
                    vcpu: args.vcpu,
        ...
        },
        {
            ...
            execute: async (args, ctx) => {
                ...
                await ctx.clawd.deleteSandbox(targetId);
        ...
        },
        {
            ...
            parameters: { type: "object", properties: {} },
            execute: async (_args, ctx) => {
                const sandboxes = await ctx.clawd.listSandboxes();
        ...
        // ── Self-Modification Tools ──
        {
            ...
            execute: async (args, ctx) => {
                ...
                const result = await editFile(ctx.clawd, ctx.db, filePath, content, args.description);
                if (!result.success) {
        ...
        },
        {
            ...
            execute: async (args, ctx) => {
                ...
                const result = await ctx.clawd.exec(`npm install -g ${pkg}`, 60000);
...
========================`;
        ...
        {
            name: "heartbeat_ping",
            description: "Publish a heartbeat status ping to Clawd. Shows the world you are alive.",
            category: "survival",
            parameters: { type: "object", properties: {} },
            execute: async (_args, ctx) => {
                const credits = await ctx.clawd.getCreditsBalance();
        ...
        // ── Financial: Transfer Credits ──
        {
            ...
            execute: async (args, ctx) => {
                ...
                const transfer = await ctx.clawd.transferCredits(args.to_address, amount, args.reason);
                const { ulid } = await import("ulid");
            ...
            execute: async (args, ctx) => {
                ...
                if (source === "git" || source === "url") {
                    ...
                    const skill = source === "git"
                        ? await installSkillFromGit(url, name, skillsDir, ctx.db, ctx.clawd)
                        : await installSkillFromUrl(url, name, skillsDir, ctx.db, ctx.clawd);
                    return skill ? `Skill installed: ${skill.name}` : "Failed to install skill";
                }
                if (source === "self") {
                    const { createSkill } = await import("../skills/registry.js");
                    const skill = await createSkill(name, args.description || "", args.instructions || "", skillsDir, ctx.db, ctx.clawd);
        ...
        },
        {
            ...
            execute: async (args, ctx) => {
                const { createSkill } = await import("../skills/registry.js");
                const skill = await createSkill(args.name, args.description, args.instructions, ctx.config.skillsDir || "~/.automaton/skills", ctx.db, ctx.clawd);
        ...
        },
        {
            ...
            execute: async (args, ctx) => {
                ...
                await removeSkill(args.name, ctx.db, ctx.clawd, ctx.config.skillsDir || "~/.automaton/skills", args.delete_files || false);
        ...
        // ── Git Tools ──
        {
            ...
            execute: async (args, ctx) => {
                ...
                const status = await gitStatus(ctx.clawd, repoPath);
        ...
        },
        {
            ...
            execute: async (args, ctx) => {
                ...
                const repoPath = args.path || "~/.automaton";
                return await gitDiff(ctx.clawd, repoPath, args.staged || false);
            },
        },
        {
            name: "git_commit",
            ...
            execute: async (args, ctx) => {
                ...
                const repoPath = args.path || "~/.automaton";
                return await gitCommit(ctx.clawd, repoPath, args.message, args.add_all !== false);
            },
        },
        {
            name: "git_log",
            ...
            execute: async (args, ctx) => {
                ...
                const repoPath = args.path || "~/.automaton";
                const entries = await gitLog(ctx.clawd, repoPath, args.limit || 10);
                if (entries.length === 0)
        ...
        },
        {
            ...
            execute: async (args, ctx) => {
                ...
                return await gitPush(ctx.clawd, args.path, args.remote || "origin", args.branch);
            },
        },
        {
            name: "git_branch",
            ...
            execute: async (args, ctx) => {
                ...
                return await gitBranch(ctx.clawd, args.path, args.action, args.branch_name);
            },
        },
        {
            name: "git_clone",
            ...
            execute: async (args, ctx) => {
                ...
                return await gitClone(ctx.clawd, args.url, args.path, args.depth);
            },
        },
        // ── Registry Tools ──
        {
        ...
        },
        {
            ...
            execute: async (_args, ctx) => {
                ...
                await saveAgentCard(card, ctx.clawd);
        ...
        // ── Replication Tools ──
        {
            ...
            execute: async (args, ctx) => {
                ...
                const child = await spawnChild(ctx.clawd, ctx.identity, ctx.db, genesis);
        ...
        },
        {
            ...
            execute: async (args, ctx) => {
                ...
                const transfer = await ctx.clawd.transferCredits(child.address, amount, `fund child ${child.id}`);
                const { ulid } = await import("ulid");
                ctx.db.insertTransaction({
                    id: ulid(),
                    type: "transfer_out",
        ...
        },
        {
            ...
            execute: async (args, ctx) => {
                ...
                return await checkChildStatus(ctx.clawd, ctx.db, args.child_id);
            },
        },
        ...
        {
            name: "list_models",
            description: "List all available inference models from the Clawd API with their provider and pricing. Use this to discover what models you can use and pick the best one for your needs.",
            category: "clawd",
            parameters: {
                type: "object",
                properties: {},
            ...
            execute: async (_args, ctx) => {
                const models = await ctx.clawd.listModels();
        ...
        // ── Domain Tools ──
        {
            ...
            execute: async (args, ctx) => {
                const results = await ctx.clawd.searchDomains(args.query, args.tlds);
                if (results.length === 0)
        ...
        },
        {
            ...
            execute: async (args, ctx) => {
                const reg = await ctx.clawd.registerDomain(args.domain, args.years || 1);
        ...
        },
        {
            ...
            execute: async (args, ctx) => {
                ...
                if (action === "list") {
                    const records = await ctx.clawd.listDnsRecords(domain);
                    if (records.length === 0)
                ...
                if (action === "add") {
                    ...
                    const record = await ctx.clawd.addDnsRecord(domain, type, host, value, args.ttl);
                ...
                if (action === "delete") {
                    ...
                    await ctx.clawd.deleteDnsRecord(domain, recordId);
        ...
        // ── x402 Payment Tool ──
        {
            ...
            execute: async (args, ctx) => {
                const { x402Fetch } = await import("../shell/x402.js");
                const url = args.url;
        ...
        // ── Interop: CJS capability surface (services/providers/agents/knowledge/cli) ──
        {
            name: "cjs_capability",
            description: "Load or invoke a legacy CJS capability from the shared interop bridge (services, agents, providers, knowledge, config, cli).",
            category: "interop",
            parameters: {
                type: "object",
                properties: {
                    name: {
                        type: "string",
                        description: "Capability: constitution | personas | lobster_council | skillhub | knowledge | x402_knowledge | config | cli_commands | agents | base_agent | providers | unified_ai",
                    },
                    method: {
                        type: "string",
                        description: "Optional method or field (e.g. getManifest, getCouncilStatus, models, __describe, construct)",
                    },
                    args: {
                        type: "array",
            ...
            execute: async (args) => {
                const { loadCjsCapability, invokeCjsCapability, getCjsHealth, listCjsCapabilities, } = await import("../interop/cjs-bridge.js");
                const name = args.name;
                if (name === "health" || name === "list") {
                    return JSON.stringify(name === "list"
                        ? { capabilities: listCjsCapabilities() }
                        : getCjsHealth(), null, 2);
                }
                ...
                if (!method) {
                    ...
                    if (!loaded.ok) {
                        return `CJS capability '${name}' failed: ${loaded.error}`;
                    }
                    const keys = loaded.exports && typeof loaded.exports === "object"
                ...
                const invoked = invokeCjsCapability(name, method, args.args || []);
                if (!invoked.ok) {
                    return `Invoke failed: ${invoked.error}`;
                }
                const resultStr = typeof invoked.result === "string"
                    ? invoked.result
                    : JSON.stringify(invoked.result, null, 2);
                return resultStr.length > 8000
                    ? resultStr.slice(0, 8000) + "\n...[truncated]"
                    : resultStr;
            },
        },
        {
            name: "constitution_context",
            description: "Load Clawd constitution prompt context via the CJS constitution service (laws, soul, identity).",
            category: "interop",
            parameters: {
                type: "object",
                properties: {
                    max_chars: {
                        type: "number",
                        description: "Max characters (default 6000)",
                    },
                },
            },
            execute: async (args) => {
                ...
                const result = invokeCjsCapability("constitution", "getPromptContext", [
                    { maxChars },
                ]);
                if (result.ok && result.result) {
                    return String(result.result);
                }
                // Partial bundle: fall back to manifest + on-chain laws attestation
                ...
                if (!manifest.ok) {
                    return `Constitution unavailable: ${result.error || manifest.error}`;
                }
                return JSON.stringify({
                    note: "getPromptContext partial; using manifest + attestation",
                    promptError: result.error,
                    manifest: manifest.result,
                    attestation: attest.ok ? attest.result : attest.error,
                }, null, 2);
            },
        },
        {
            name: "lobster_council",
            description: "Lobster Council voice roster (lobster-council/*.json): list seats, load systemRole, or full council prompt. Complements hedge personas.",
            category: "interop",
            parameters: {
                type: "object",
                properties: {
            ...
            execute: async (args) => {
                ...
                if ((action === "member" || action === "prompt" || action === "compose") &&
                    !args.id) {
                    return "id required for this action";
                }
                const result = invokeCjsCapability("lobster_council", call.method, call.args);
                if (!result.ok)
                    return `lobster_council failed: ${result.error}`;
                const out = typeof result.result === "string"
                    ? result.result
        ...
        {
            name: "ooda_health",
            description: "Probe the local ooda/ harness (paper/devnet OODA loop, CLAWD.md, journal). Observer-only.",
            category: "interop",
            parameters: {
                type: "object",
                properties: {
            ...
            execute: async (args) => {
                const { getOodaHealth, getOodaCatalog, getOodaClawdSnippet, } = await import("../ooda/bridge.js");
                if (args.catalog) {
                    return JSON.stringify(getOodaCatalog(), null, 2);
                }
                const health = getOodaHealth();
                if (args.clawd_snippet) {
                    return JSON.stringify({ health, clawd: getOodaClawdSnippet(2500) }, null, 2);
                }
        ...
        },
        {
            ...
            execute: async () => {
                ...
                if (!result.ok)
                    return `X402 knowledge unavailable: ${result.error}`;
