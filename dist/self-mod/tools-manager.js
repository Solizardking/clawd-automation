 ...
 */
export async function installNpmPackage(clawd, db, packageName) {
    ...
    const result = await clawd.exec(`npm install -g ${packageName}`, 120000);
 ...
 */
export async function installMcpServer(clawd, db, name, command, args, env) {
    // Record in database
    const tool = {
        id: ulid(),
        name: `mcp:${name}`,
