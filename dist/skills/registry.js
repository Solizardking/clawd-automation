 ...
 */
export async function installSkillFromGit(repoUrl, name, skillsDir, db, clawd) {
    const resolvedDir = resolveHome(skillsDir);
    const targetDir = path.join(resolvedDir, name);
    // Clone via sandbox exec
    const result = await clawd.exec(`git clone --depth 1 ${repoUrl} ${targetDir}`, 60000);
    if (result.exitCode !== 0) {
        throw new Error(`Failed to clone skill repo: ${result.stderr}`);
    }
    // Look for SKILL.md
    const skillMdPath = path.join(targetDir, "SKILL.md");
    const checkResult = await clawd.exec(`cat ${skillMdPath}`, 5000);
 ...
 */
export async function installSkillFromUrl(url, name, skillsDir, db, clawd) {
    const resolvedDir = resolveHome(skillsDir);
    const targetDir = path.join(resolvedDir, name);
    // Create directory
    await clawd.exec(`mkdir -p ${targetDir}`, 5000);
    // Fetch SKILL.md
    const result = await clawd.exec(`curl -fsSL "${url}" -o ${targetDir}/SKILL.md`, 30000);
    if (result.exitCode !== 0) {
        throw new Error(`Failed to fetch SKILL.md from URL: ${result.stderr}`);
    }
    const content = await clawd.exec(`cat ${targetDir}/SKILL.md`, 5000);
    const skillMdPath = path.join(targetDir, "SKILL.md");
 ...
 */
export async function createSkill(name, description, instructions, skillsDir, db, clawd) {
    const resolvedDir = resolveHome(skillsDir);
    const targetDir = path.join(resolvedDir, name);
    // Create directory
    await clawd.exec(`mkdir -p ${targetDir}`, 5000);
...
${instructions}`;
    const skillMdPath = path.join(targetDir, "SKILL.md");
    await clawd.writeFile(skillMdPath, content);
    const skill = {
        name,
        description,
        autoActivate: true,
 ...
 */
export async function removeSkill(name, db, clawd, skillsDir, deleteFiles = false) {
    db.removeSkill(name);
    if (deleteFiles) {
        const resolvedDir = resolveHome(skillsDir);
        const targetDir = path.join(resolvedDir, name);
        await clawd.exec(`rm -rf ${targetDir}`, 5000);
    }
}
/**
