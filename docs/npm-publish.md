...
| Check | Status |
|-------|--------|
| Name / version | `@clawd/automaton@0.1.0` |
...
Installer prefers `npm install -g @clawd/automaton@…` when the package exists on the registry; otherwise clones + builds into `~/.local/share/clawd-automaton`.

## Registry publish (requires your credentials)

This environment has **no valid npm auth** (`npm whoami` → `401`). The `@clawd` npm org also does **not** exist yet.

### 1. Create scope + login (once)

1. Sign in at https://www.npmjs.com/
2. Create organization **`clawd`** (or claim the `@clawd` scope)
3. On this machine:
...
Token needs **publish** permission on `@clawd/*`.
...
npm view @clawd/automaton version   # → 0.1.0
```

### 3. Consumer verify

```bash
npm install -g @clawd/automaton@0.1.0
...
# or: node scripts/verify-pack.mjs /tmp/evidence
```
