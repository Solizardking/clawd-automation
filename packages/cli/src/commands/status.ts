/**
 * automaton-cli status — show local automaton state
 */

import { loadConfig, resolvePath } from "@onchainai/automation/config.js";
import { createDatabase } from "@onchainai/automation/state/database.js";
...
if (recentTurns.length > 0) {
  ...
  for (const turn of recentTurns) {
    ...
    console.log(
      `  [${turn.timestamp}] ${turn.thinking.slice(0, 80)}...${toolNames ? ` (tools: ${toolNames})` : ""}`,
    );
  }
}
