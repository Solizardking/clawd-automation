...
describe("CJS interop bridge", () => {
  ...
  it("loads personas list from CJS service", () => {
    ...
    expect(result.ok).toBe(true);
    const body = result.result as { name?: string; personas?: unknown[] } | Record<string, unknown>;
  ...
  it("loads agents package (agent-council) and invokes getCouncilStatus", () => {
    ...
    const shape = desc.result as { kind: string; methods: string[]; keys: string[] };
    expect(shape.kind).toBe("object");
    expect(shape.methods).toContain("getCouncilStatus");
    expect(shape.keys).toEqual(
      expect.arrayContaining(["agents", "sessions", "votingHistory"]),
    );
  ...
  it("loads base_agent class and constructs a real instance", () => {
    ...
    expect((loaded.exports as { name?: string }).name).toBe("BaseAgent");
    ...
    const body = constructed.result as {
      constructed: boolean;
      className: string;
      name: string;
      type: string;
  ...
  it("loads providers package (openrouter) and reads models field", () => {
    ...
    const map = models.result as Record<string, string>;
    ...
    expect(desc.ok).toBe(true);
    const shape = desc.result as { methods: string[] };
  ...
  it("loads unified_ai provider and describes export shape", () => {
    ...
    expect(desc.ok).toBe(true);
    const shape = desc.result as { kind: string; keys: string[]; methods: string[] };
    expect(shape.kind).toBe("object");
    expect(shape.keys).toEqual(
      expect.arrayContaining(["openRouter", "cloudflare", "agentName"]),
    );
  ...
  it("resolveSrcRoot finds src/ from a simulated dist/interop layout", () => {
    ...
    expect(existsSync(path.join(repoSrc, "services", "constitution.js"))).toBe(
      true,
    );
    ...
    expect(
      existsSync(path.join(resolved, "providers", "openrouter.js")),
    ).toBe(true);
    expect(existsSync(path.join(resolved, "agents", "agent-council.js"))).toBe(
      true,
    );
  });
  ...
  it("CJS config export has openRouter (not ESM Automaton config.ts shape)", () => {
    ...
    expect(loaded.ok).toBe(true);
    const cfg = loaded.exports as {
      openRouter?: { models?: Record<string, string> };
      product?: { name?: string };
    };
...
describe("Cross-package tool dispatch", () => {
  ...
  it("executeTool(cjs_capability) loads constitution via bridge", async () => {
    ...
    const result = await executeTool(
      "cjs_capability",
      { name: "constitution", method: "getManifest" },
      runtime.tools,
      ctx,
    );
  ...
  it("executeTool(cjs_capability) loads agents package on primary tool path", async () => {
    ...
    const result = await executeTool(
      "cjs_capability",
      { name: "agents", method: "getCouncilStatus" },
      runtime.tools,
      toToolContext(runtime),
    );
