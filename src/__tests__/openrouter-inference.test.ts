...
import {
  createOpenRouterInferenceClient,
  createOpenRouterFromEnv,
  resolveOpenRouterEnv,
  isOpenRouterConfigured,
  OPENROUTER_FREE_ROUTER,
} from "../inference/openrouter.js";
import {
...
describe("isOpenRouterConfigured / createOpenRouterFromEnv", () => {
  it("detects missing key", () => {
    expect(isOpenRouterConfigured({} as any)).toBe(false);
    expect(createOpenRouterFromEnv({} as any)).toBeNull();
  });
...
describe("resolveInferenceBackend", () => {
  it("defaults to auto", () => {
    expect(resolveInferenceBackend({} as any)).toBe("auto");
  });
});
