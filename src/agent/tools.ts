  ...
  return [
    // ── VM/Sandbox Tools ──
    {
      ...
      execute: async (args, ctx) => {
        ...
        const result = await ctx.clawd.exec(
          command,
    ...
    },
    {
      ...
      execute: async (args, ctx) => {
        ...
        await ctx.clawd.writeFile(filePath, args.content as string);
    ...
    },
    {
      ...
      execute: async (args, ctx) => {
        return await ctx.clawd.readFile(args.path as string);
      },
    },
    {
      name: "expose_port",
      ...
      execute: async (args, ctx) => {
        const info = await ctx.clawd.exposePort(args.port as number);
    ...
    },
    {
      ...
      execute: async (args, ctx) => {
        await ctx.clawd.removePort(args.port as number);
    ...
    },
    {
      name: "create_sandbox",
      description:
        "Create a new Clawd sandbox (separate VM) for sub-tasks or testing.",
      category: "clawd",
      parameters: {
        type: "object",
        properties: {
...
========================`;
    ...
    },
    {
      name: "heartbeat_ping",
      description:
        "Publish a heartbeat status ping to Clawd. Shows the world you are alive.",
      category: "survival",
      parameters: { type: "object", properties: {} },
      execute: async (_args, ctx) => {
        const credits = await ctx.clawd.getCreditsBalance();
    ...
    // ── Financial: Transfer Credits ──
    {
      ...
      execute: async (args, ctx) => {
        ...
        const transfer = await ctx.clawd.transferCredits(
    ...
    },
    {
      ...
      execute: async (args, ctx) => {
        ...
        const repoPath = (args.path as string) || "~/.automaton";
        return await gitDiff(ctx.clawd, repoPath, (args.staged as boolean) || false);
      },
    },
    {
      name: "git_commit",
      ...
      execute: async (args, ctx) => {
        ...
        const repoPath = (args.path as string) || "~/.automaton";
        return await gitCommit(ctx.clawd, repoPath, args.message as string, args.add_all !== false);
      },
    },
    {
      name: "git_log",
      ...
      execute: async (args, ctx) => {
        ...
        const repoPath = (args.path as string) || "~/.automaton";
        const entries = await gitLog(ctx.clawd, repoPath, (args.limit as number) || 10);
    ...
    },
    {
      ...
      execute: async (args, ctx) => {
        ...
        return await checkChildStatus(ctx.clawd, ctx.db, args.child_id as string);
      },
    },
    ...
    // ── Model Discovery ──
    {
      name: "list_models",
      description:
        "List all available inference models from the Clawd API with their provider and pricing. Use this to discover what models you can use and pick the best one for your needs.",
      category: "clawd",
      parameters: {
        type: "object",
        properties: {},
    ...
    // ── Domain Tools ──
    {
      ...
      execute: async (args, ctx) => {
        const results = await ctx.clawd.searchDomains(
    ...
    },
    {
      ...
      execute: async (args, ctx) => {
        const reg = await ctx.clawd.registerDomain(
    ...
    },
    {
      ...
      execute: async (args, ctx) => {
        ...
        if (action === "add") {
          ...
          const record = await ctx.clawd.addDnsRecord(
            domain,
            type,
            host,
            value,
    ...
    // ── Interop: CJS capability surface (services/providers/agents/knowledge/cli) ──
    {
      name: "cjs_capability",
      description:
        "Load or invoke a legacy CJS capability from the shared interop bridge (services, agents, providers, knowledge, config, cli).",
      category: "interop",
      parameters: {
        type: "object",
        properties: {
          ...
          method: {
            type: "string",
            description:
              "Optional method or field (e.g. getManifest, getCouncilStatus, models, __describe, construct)",
          },
          args: {
            type: "array",
      ...
      execute: async (args) => {
        const {
          ...
          getCjsHealth,
          listCjsCapabilities,
        } = await import("../interop/cjs-bridge.js");
        const name = args.name as string;
        ...
        const invoked = invokeCjsCapability(
          name as any,
          method,
    ...
    },
    {
      ...
      description:
        "Load Clawd constitution prompt context via the CJS constitution service (laws, soul, identity).",
      category: "interop",
      parameters: {
        type: "object",
        properties: {
    ...
    },
    {
      name: "ooda_run",
      description:
        "Run a short deterministic paper OODA cycle (synth candles, no LLM, no mainnet). Max 50 ticks.",
      category: "interop",
      parameters: {
        type: "object",
        properties: {
    ...
    },
    {
      name: "ooda_decide",
      description:
        "One-shot paper decision via ooda deterministic SMA logic (no LLM, no funds).",
      category: "interop",
      parameters: {
        type: "object",
        properties: {
      ...
      execute: async (args) => {
        ...
        const closes = Array.isArray(args.closes)
          ? (args.closes as number[])
          : undefined;
        const result = await oodaDecide({
          closes,
          cash_lamports:
            typeof args.cash_lamports === "number"
              ? args.cash_lamports
              : undefined,
        });
        return JSON.stringify(result, null, 2);
    ...
    },
    {
      name: "ooda_journal",
      description:
        "Read the last N entries from ooda/journal/ticks.jsonl (paper harness memory).",
      category: "interop",
      parameters: {
        type: "object",
        properties: {
    ...
    },
    {
      name: "zk_health",
      description:
        "Probe the local zk-primitives tree (MANIFEST, client, agent, program id, env). Observer-only — never signs txs.",
      category: "interop",
      parameters: { type: "object", properties: {} },
      execute: async () => {
    ...
    },
    {
      name: "zk_catalog",
      description:
        "Return the Clawd ZK primitives catalog (operations, packages, trust gates, docs paths).",
      category: "interop",
      parameters: {
        type: "object",
        properties: {
      ...
      execute: async (args) => {
        const { getZkCatalog, getZkPromptContext } = await import(
          "../zk/primitives.js"
        );
        if (args.as_prompt) {
    ...
    },
    {
      name: "x402_knowledge",
      description:
        "Load X402 protocol knowledge from the CJS knowledge package for agent context.",
      category: "interop",
      parameters: { type: "object", properties: {} },
      execute: async () => {
        const { invokeCjsCapability } = await import("../interop/cjs-bridge.js");
        ...
        if (!result.ok) return `X402 knowledge unavailable: ${result.error}`;
