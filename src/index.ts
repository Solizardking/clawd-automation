...
async function run(): Promise<void> {
  ...
  console.log(
    `[${new Date().toISOString()}] Shell: local clawd sandbox=${sandboxId}`,
  );
  ...
  try {
    ...
    console.log(
      `[${new Date().toISOString()}] CJS bridge: ${cjsHealth.available.length} available, ${cjsHealth.unavailable.length} unavailable`,
    );
  } catch (err: any) {
