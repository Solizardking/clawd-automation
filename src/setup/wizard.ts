...
export async function runSetupWizard(): Promise<AutomatonConfig> {
  ...
  if (apiKey) {
    ...
    console.log(
      chalk.yellow(
        "  Set OPENROUTER_API_KEY for free-router inference (https://openrouter.ai/keys).\n",
      ),
    );
    const manual = await promptRequired(
      "OpenRouter API key (sk-or-…, or Enter to skip)",
    );
  ...
  if (!apiKey) {
    console.log(
      chalk.yellow(
        "  No OpenRouter key yet. Export OPENROUTER_API_KEY before --run.\n",
      ),
    );
  }
