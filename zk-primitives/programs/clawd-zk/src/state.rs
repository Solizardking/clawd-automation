...
//! CPI to the Light System Program (one-time per tree per tx, ~5k
...
pub struct ConsumeStateData {
    pub proof: ValidityProof,
    /// Metadata and pre-state are both required to recompute the input leaf.
    pub account_meta: CompressedAccountMeta,
    pub account: AttestationAccount,
    pub system_accounts_offset: u8,
}
...
pub fn write_attestation<'info>(
    ...
    let mut account = LightAccount::<AttestationAccount>::new_init(
        &crate::ID,
        data.address,
...
pub fn consume_attestation<'info>(
    ...
    require!(account.address() == &Some(*attestation_address), ClawdZkError::UnknownTree);
...
pub fn commit_encrypted<'info>(
    ...
    let clock = Clock::get()?;
    let record = EncryptedStateAccount {
        model_hash: *model_hash,
    ...
    let mut account = LightAccount::<EncryptedStateAccount>::new_init(
        &crate::ID,
        data.address,
        data.output_state_tree_index,
    );
